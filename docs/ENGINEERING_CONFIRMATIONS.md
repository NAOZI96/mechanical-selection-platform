...
- Phase 7 软件验收：`local_pass`（八模块页面/API/快照/HTML/PDF 已实现，123 项本地回归已通过）
- Phase 7 远程部署：`not_deployed`
...
- 软件/运维：主代理最终本地复验；九模块版本目标主机资源、备份恢复、PDF 并发和既有服务影响复验。
- 公网：ICP 首次备案/接入或新增服务关闭后，重新验证国内外 HTTPS、搜索引擎抓取和外部监控。

本轮没有数据库迁移、没有新增常驻服务，也没有执行远程部署；既有 Phase 4 验收不能被解释为八模块已经远程放行。
