...
class ApiTests(unittest.TestCase):
    ...
    def test_health_module_discovery_and_schema(self) -> None:
        ...
        self.assertEqual(self.client.get("/openapi.json").json()["info"]["version"], "0.5.2")
    ...
    def test_platform_homepage_exposes_nine_registered_modules_with_release_boundaries(self) -> None:
        ...
        self.assertIn('href="/static/app.css?v=20260812.1"', response.text)
    ...
    def test_chinese_calculator_page_and_static_assets(self) -> None:
        ...
        self.assertIn('id="loading-state"', response.text)
        ...
        self.assertIn('href="/static/app.css?v=20260812.1"', response.text)
        self.assertIn('src="/static/calculator.js?v=20260812.2"', response.text)
        ...
        self.assertIn('setResultState("loading")', script.text)
        self.assertIn('setResultState("result")', script.text)
        self.assertIn("window.sessionStorage.setItem(SESSION_STORAGE_KEY", script.text)
        self.assertIn("window.sessionStorage.removeItem(SESSION_STORAGE_KEY)", script.text)
        self.assertIn('id="clear-calculator"', response.text)
