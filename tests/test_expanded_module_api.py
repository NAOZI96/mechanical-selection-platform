    ...
    def test_eight_modules_run_through_schema_page_snapshot_html_and_pdf(self) -> None:
        for spec in EXPANDED_MODULE_SPECS:
            with self.subTest(module_id=spec.module_id):
                ...
                self.assertIn('src="/static/engineering-calculator.js?v=20260725.2"', page.text)
                ...
                self.assertEqual(snapshot["report_context"]["release_status"], "internal_testing")
                self.assertEqual(snapshot["report_context"]["release_status_label"], "内部测试")
                ...
                self.assertEqual(pdf.headers["x-engineering-release-status"], "internal_testing")
                self.assertEqual(pdf.headers["x-legacy-release-status-missing"], "false")
